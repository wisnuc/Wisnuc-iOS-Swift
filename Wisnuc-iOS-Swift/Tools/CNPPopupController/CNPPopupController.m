...
#define COR1 UICOLOR_RGB(0x009788)
#define PopUpViewWidth    UIScreen.mainScreen.bounds.size.width - 32
#define Margins 16
#define MarginsSofar 24
...
@property (nonatomic, strong) UILabel *topAdressLabel;
...
//    self.popupView.backgroundColor = [UIColor cyanColor];
    CGSize result = CGSizeMake(0, TopViewHeight);
...
//    result.width += inset.left + inset.right;
    ...
    if (update) {
        ...
        self.popupView.frame = CGRectMake(0, 0, PopUpViewWidth, result.height);
...
- (void)presentPopupControllerAnimated:(BOOL)flag {
    ...
    if ([self.delegate respondsToSelector:@selector(addressString)]) {
       self.topAdressLabel.text = [self.delegate addressString];
    }
    
    if ([self.delegate respondsToSelector:@selector(nameString)]) {
        self.topNameLabel.text = [self.delegate nameString];
    }
...
- (UIView *)topView{
    if (!_topView) {
        _topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0,PopUpViewWidth , TopViewHeight)];
        _topView.backgroundColor = COR1;
        [_topView addSubview:self.topAdressLabel];
        [_topView addSubview:self.topNameLabel];
    }
    return _topView;
}

- (UILabel *)topAdressLabel{
    if (!_topAdressLabel) {
        _topAdressLabel = [[UILabel alloc]initWithFrame:CGRectMake(MarginsSofar, MarginsSofar,PopUpViewWidth - MarginsSofar *2 , 16)];
        _topAdressLabel.font = [UIFont boldSystemFontOfSize:14];
        _topAdressLabel.textColor = [UIColor colorWithWhite:1-0.2f alpha:1];
    }
...
- (UILabel *)topNameLabel{
    if (!_topNameLabel) {
        _topNameLabel = [[UILabel alloc]initWithFrame:CGRectMake(MarginsSofar, _topAdressLabel.bottom + Margins,PopUpViewWidth - MarginsSofar *2, 36)];
        _topNameLabel.font = [UIFont boldSystemFontOfSize:34];
